...
{.-}
help for {cmd:mdata stats} {right:}
{.-}

{title:Title}

{pstd}
{cmd:mdata stats} {hline 1} extract statistics to metadata files
...
the command will first extract the metadata (see {help mdata_extract:mdata extract}) 
...
{pstd}
{opt mdata stats} is a Stata command that computes variables' 
statistics/frequencies and exports them to an Excel file. It is part 
of the {help mdata} package.

{pstd}
The user may use an existing metadata file created with 
{help mdata_extract:mdata extract} (see option {opt metafile}) 
...
{opt mdata stats} calls {help mdata_extract:mdata extract} to save the 
...
{p 8 16}{inp:. mdata stats, save(meta_auto)}{p_end}

{pstd}
Change metadata file "meta_auto.xlsx" by adding all possible statistics.  

{p 8 16}{inp:. sysuse auto, clear}{p_end}
{p 8 16}{inp:. mdata extract, meta(meta_auto)}{p_end}
{p 8 16}{inp:. mdata stats, meta(meta_auto) stats(_all)}{p_end}
...
{p 8 16}{inp:. mdata stats, save(meta_nls) time(year) panel(idcode)}{p_end}
...
{p 8 16}{inp:. webuse nlswork, clear}{p_end}
{p 8 16}{inp:. mdata stats, save(meta_nls) time(year) panel(idcode) exc(idcode year) nof(race)}{p_end}
