...
{.-}
help for {cmd:mdata check} {right:}
{.-}

{title:Title}

{pstd}
{cmd:mdata check} {hline 1} checks for inconsistencies in metadata
...
use of {help mdata_apply:mdata apply}, which performs an integrity check of 
the metadata before applying it. So an inconsistency is every problem found in the 
Excel file that stops {help mdata_apply:mdata apply} from running. On the other 
hand, warnings flag problems in the metadata, but do not halt the execution of 
{help mdata_apply:mdata apply}.
...
{pstd}
{cmd:mdata check} stores the following in {cmd:r()}:
...
{p 8 16}{inp:. mdata check, meta(meta_auto)}{p_end}
...
{p 8 16}{inp:. mdata check, meta(meta_auto) check(checkfile)} del(" "){p_end}
