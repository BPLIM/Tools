...
{.-}
help for {cmd:metaxl diff} {right:}
{.-}

{title:Title}

{pstd}
{cmd:metaxl diff} {hline 1} flags differences in metadata files
...
{pstd}
{opt metaxl diff} is a Stata command that performs a consistency 
...
{p 8 16}{inp:. metaxl diff, path(metadir)}{p_end}
...
{p 8 16}{inp:. metaxl diff, path(metadir) basefile(meta02.xlsx)}{p_end}
...
{p 8 16}{inp:. metaxl diff, path(metadir) save(metadiff, replace) diffonly}{p_end}
