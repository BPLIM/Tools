...
forvalues j= 2/5 {
	preserve
		...
		local total = r(N)
		if "`en'" == "en" {
			label define divlabel2`j' -99 "Unsuccessful conversion", add modify 
			label define divlabel2`j' -98 "Ambiguous validation, not able to convert", add modify 
		}
		else {
			label define divlabel2`j' -99 "Conversão indisponível", add modify 
			label define divlabel2`j' -98 "Validação ambígua, código não convertido", add modify 		
		}
		forvalues i = 1/`total' {
